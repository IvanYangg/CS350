...
static void handle_connection(int conn_socket)
{
    ...
    while (1) {
        ...
        ssize_t bytes_received = recv(conn_socket, &req, sizeof(struct request), 0);
		...
		ssize_t bytes_sent = send(conn_socket, &req.request_id, sizeof(uint64_t), 0);
        clock_gettime(CLOCK_MONOTONIC, &completion_timestamp);
        if (bytes_sent == -1) {
            perror("Error sending response");
            break; 
        }
		printf("R%lu: %.9lf,%.9lf,%.9lf,%.9lf\n", req.request_id, TSPEC_TO_DOUBLE(req.sent_timestamp), TSPEC_TO_DOUBLE(req.request_length), TSPEC_TO_DOUBLE(receipt_timestamp), TSPEC_TO_DOUBLE(completion_timestamp));
    }
}
...
int main (int argc, char ** argv) {
	...
	/* returns -1 if socket fails, so we check for this */
	if (sockfd < 0) {
		ERROR_INFO();
