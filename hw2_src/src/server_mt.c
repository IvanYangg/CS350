...
void handle_connection(int conn_socket)
{
    ...
    int res = clone(worker_main, child_stack + STACK_SIZE, CLONE_THREAD | CLONE_VM | CLONE_SIGHAND | CLONE_FS | CLONE_FILES | CLONE_SYSVSEM, 0);
	/* error checking */
    if (res == -1)
    {
        perror("clone");
    ...
    while (1) {
		...
		ssize_t bytes_sent = send(conn_socket, &req.req_id, sizeof(uint64_t), 0);
