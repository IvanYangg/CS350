...
int add_to_queue(struct timeRequest to_add, struct queue * the_queue)
{
	...
	if (the_queue->size >= QUEUE_SIZE) {
    	...
    	response.req_id = to_add.request.req_id;
    	response.status = 1;  // 1 for rejected
    	// Send the rejection response to the client
    	// You need to implement the code for sending responses to the client.
...
{
	int res = clone(worker_main, worker_stack + STACK_SIZE, CLONE_THREAD | CLONE_VM | CLONE_SIGHAND | CLONE_FS | CLONE_FILES | CLONE_SYSVSEM, &params);
}
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	/* IMPLEMENT ME !!*/
	//conn_params holds size of queue, so we can send that as q_size for the queue init helper function and call that to handle queue allocation and initialization. 

	/* Prepare worker_parameters */
	worker_params.serverQueue = the_queue;
